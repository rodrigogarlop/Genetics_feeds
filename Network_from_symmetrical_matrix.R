# Update 2021-03-18: Added an exception to avoid running the two column net construction if conditions are not met: Two metadata columns are required: first one must be numeric, second one categorical (as factor)
...
# LIBRARIES REQUIRED: igraph, RColorBrewer, reshape2
...
# Layout can be set to either Fruchterman-Reingold or a sphere grid when setting variable: "lo <-"
# IMPORTANT NOTE 3: An additional output graphic, with suffix "full-groups_twocolmeta.pdf" is created if the following special conditions are met: Two metadata columns are required: one numeric, one categorical (nominal). This will trigger a special case for constructing a large networks and was originally intended to use total abundance and taxa for OTUs/ASVs, but can be used with any pair of numerical + categorical columns.

# Run as follows:
# cat symmetrical_matrix.tsv|Rscript Network_from_symmetrical_matrix.R <metadata_table.tsv> <prefix_output> <TRUE/FALSE_if_correlation_matrix> <TRUE/FALSE_to_print_names> [min_value (optional)]
# Tested with command:
...
# Internally: 
# df <- as.matrix(read.table("/home/rod/Documents/02_Collaborations/Geneticas/03_corr_tables/genetics_lvlOTU-H_grp_A-corr_pearson_rows_maxpval_0.001.tsv", sep="\t",header=T, skip=0, comment.char='',quote="",fill=F, row.names=1, check.names=F))
# meta <- "01_input_tables/xref-OTUs_phyla.tsv"
# print_names <- FALSE
# is_corr <- TRUE # as.logical(0)
# prefix <- "test"
# min_val <- 0.8
...
if(sum(is.na(match(rownames(df),rownames(mdf))))>0) {stop(paste0("Aborting: Items are missing from the metadata file. Please check if all items are present."), call.=FALSE)} # Abort if the metadata file is not present
temp <- colnames(mdf)
...
# 	dist_mat <- distances(in_net) # Optionally, get the total distance for each two nodes (redundant)
	...
	eigen <- centr_eigen(in_net, directed=F, normalized=T) # only this one needs to be filtered
...
# 	centrality <- cbind(centrality, "authority.score"=c(authority_score(in_net, weights=NA)$vector,NA,NA)) # same as hubs if undirected
	...
	subgraphs <- decompose.graph(in_net, mode = "strong", max.comps = NA, min.vertices = 10)
	subgraphs.gt10.nodes <- length(subgraphs) # Get how many subgraphs larger than 10 exist
	if (subgraphs.gt10.nodes>0) {
		subgraphs <- subgraphs[order(unlist(lapply(subgraphs, gorder)),decreasing=T)] # Sort the list by total vertices
		Component <- rep(1:length(subgraphs),unlist(lapply(subgraphs, gorder))) # Produce the subgraphs
		Vertex.ID <- names(unlist(lapply(subgraphs, V)))
		Vertex.Name <- new_names[Vertex.ID]
		subgraphs.out <- cbind(Component, Vertex.ID, Vertex.Name)
		write.table(subgraphs.out,paste0(prefix,"-cut-",min_val,"-",pre,"-subgraphs.tsv"), sep="\t", quote=FALSE, col.names=T, row.names=F)
	} else {
		subgraphs.gt10.nodes <- 0
	}
	global_specs <- data.frame(min_val, nodes, edges, filt_mean_weight, filt_weight_sd, filt_weight_median, dens, diam, mean_dist, subgraphs.gt10.nodes, "nodes_in_diam"=paste(V(in_net)$name[get_diameter(in_net)], collapse='-'))
...
# 	lo <- layout_with_fr(in_net) # Create a fixed layout for all Fruchterman and Reingold graphs
	...
	for(i in 1:ncol(metadata)){
		...
		sizes <- 15 # set default size to 15
		...
		if(is.factor(items)){
			...
			legend("topleft", legend = levels(items), pch=15, pt.cex=0.6, cex=0.4, bty='n', col = node_cols) # create the labels
			mtext("Groups", at=0.2, cex=1) #add some title
		}
		plot(net, vertex.color=sapply(temp, function(x) node_cols[x]), vertex.frame.color="white", vertex.label.color="black", vertex.label.family="Helvetica", main=paste0("Network depicting group: ", names(metadata)[i]), edge.width=rescale_vect(E(in_net)$weight,0.5,2), vertex.size=sizes, edge.color=edge_cols,layout=layout.circle(in_net, order=order(items)))
		mtext(paste0("Cutoff value: ", min_val))
	}
	dev.off()
}
# plot_meta(net, mdf, "full")
plot_meta_two_cols <- function(in_net, metadata, pre){ # this was a special case with groups in col 1 and a continuous numeric variable in col 2 of the metadata file (the header has: #OTU ID Phylum  Rel_Sum)
# 	lo_sphere <- layout_on_sphere(in_net) # alternative case where sphere grid is required
	range <- round(seq(-1,1,0.01),2) # Create a range for expected rho values (201 items, from -1 to 1 by default)
	temp <- colnames(metadata)
	deg <- degree(in_net)
	metadata <- as.data.frame(metadata[V(in_net)$name,]) # subset the metadata table in case some nodes were unconnected
...
# 	lo <- layout_with_fr(in_net) # Create a fixed layout for all Fruchterman and Reingold graphs
	pdf(paste0(prefix,"-cut-",min_val,"-",pre,"-groups_twocolmeta.pdf"))
	i=1
	node_cols <- set_node_col(1) # By default, set to 1 color (if no factors are present)
	...
	if(is.numeric(metadata[,2])){
		sizes <- rescale_vect(metadata[,2],3,10)
	}
	plot(net, vertex.color=sapply(temp, function(x) node_cols[x]), vertex.frame.color="white", vertex.label.color="black", vertex.label.family="Helvetica", main=paste0("Network depicting group: ", names(metadata)[i]), edge.width=0.5, vertex.size=sizes, edge.color=edge_cols, layout=lo,  vertex.label=NA)
	mtext(paste0("Cutoff value: ", min_val, " Size: overall prevalence in set"))
	plot(NULL ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1) # create empty canvas
	legend("topleft", legend = levels(items), pch=15, pt.cex=0.6, cex=0.4, bty='n', col = node_cols) # create the labels
	mtext("Groups", at=0.2, cex=1) #add some title
	plot(net, vertex.color=sapply(temp, function(x) node_cols[x]), vertex.frame.color="white", vertex.label.color="black", vertex.label.family="Helvetica", main=paste0("Network depicting group: ", names(metadata)[i]), edge.width=0.5, vertex.size=sizes, edge.color=edge_cols, layout=layout.circle(in_net, order=order(deg)),  vertex.label=NA)
	mtext(paste0("Cutoff value: ", min_val, " Vertices sorted by connectivity degree"))
	plot(net, vertex.color=sapply(temp, function(x) node_cols[x]), vertex.frame.color="white", vertex.label.color="black", vertex.label.family="Helvetica", main=paste0("Network depicting group: ", names(metadata)[i]), edge.width=0.5, vertex.size=sizes, edge.color=edge_cols, layout=layout.circle(in_net, order=order(items)),  vertex.label=NA)
	mtext(paste0("Cutoff value: ", min_val, "Vertices sorted by group"))
	dev.off()
}
...
# 	clu_edg_btw <- cluster_edge_betweenness(in_net, directed=F, weights=1.01-E(in_net)$scaled_01) # cluster based on edge betweenness (Newman-Girvan). # NOTE. Large graphs will lead to segmentation fault.
...
plot_meta(net, mdf, "full")
if((is.factor(mdf[1,1])) & (is.numeric(mdf[1,2]))){ # If the first column in the metadata file is numerical an the second one is categorical
	plot_meta_two_cols(net, mdf, "full") # added a special case with no labels on a grid for large sets (uncomment if required), see note at the top of this file
}
# 5.- Community detection
# community_detection(net,"full")
community_detection_for_large_graph(net,"full") # Use this if the network has over 300 nodes as it will lead to a segmentation fault (core dump). This avoids calculation of edge betweenness (both edge-wise and for the clustering algorithm).
