...
plot_boxplot <- function(tab){ # from a 2-column table, plot densities
	a <- tab[,1][!is.na(tab[,1])]
	b <- tab[,2][!is.na(tab[,2])]
# 	range.x <- range(c(A[[1]],B[[1]]))
# 	max.y <- max(c(A[[2]],B[[2]]))
	pval <- wilcox.test(a,b, paired=T)$p.value # This can be changed to Mann-Whitney by using paired=FALSE
	pval2 <- t.test(a,b, paired=T)$p.value
	if(pval < 0.0001){pval = "< 0.0001"}else{pval=round(pval,4)}
	if(pval2 < 0.0001){pval2 = "< 0.0001"}else{pval2=round(pval2,4)}
	boxplot(a,b, col=t_cols, frame.plot=F, border=solid_cols, main=names(tab)[1], xaxt='n', outline=T)
	axis(1,las=1, labels=c("Genetics 1","Genetics 2"), at=1:2)
	legend("topright", legend=(c(paste0("Mann-Whitney p-value: ",pval), paste0("paired t p-value: ",pval2))), bty = "n")
# 	plot(A,B)
}
plot_ggboxplot <- function(tab) {
	...
	boxplot <- ggplot(long_format, aes(x = ind, y = values, color = ind)) + 
		...
		labs(title= names(tab)[1], x = "", y = "") + scale_color_manual(values = colors) + 
		stat_compare_means(comparisons = p_value, method = "wilcox.test")
	print(boxplot)
}
plot_ggboxplot_no_outliers <- function(tab) {
	p_value <- list(c("Genetics 1", "Genetics 2"))
	long_format <- stack(tab)
	minvect <- quantile(tab[,order(apply(tab,2,function(x) quantile(x)[2]))[1]]) # get which column has the smallest min and get its quantiles
	maxvect <- quantile(tab[,order(apply(tab,2,function(x) quantile(x)[4]),decreasing=TRUE)[1]]) # get which column has the largest max and get its quantiles
	upper.limit <- maxvect[4] + 1.5*(maxvect[4]-maxvect[2]) # Get what may be considered outliers
	lower.limit <- minvect[2] - 1.5*(minvect[4]-minvect[2]) 
	upper.limit <- ifelse(upper.limit>max(tab),max(tab),upper.limit) # Fix if out of boundaries (cap to max or min values)
	lower.limit <- ifelse(lower.limit<min(tab),min(tab),lower.limit)
	levels(long_format[,2]) <- c("Genetics 1","Genetics 2")
	boxplot <- ggplot(long_format, aes(x = ind, y = values, color = ind)) + 
		geom_boxplot(outlier.shape=NA) + theme_test() + 
		coord_cartesian(ylim=c(lower.limit, upper.limit)) +
		theme(legend.position = "none", text = element_text(size = 16), axis.title.y = element_text(size = 18), axis.text.x = element_text(size = 18), plot.title = element_text(hjust = 0.5, size=22)) + 
		labs(title= names(tab)[1], x = "", y = "") + scale_color_manual(values = colors)
# 		stat_compare_means(comparisons = p_value, method = "wilcox.test") # This is often out of frame
	print(boxplot)
}
process_column <- function(column=2,organ){ # this is just to circle through columns and plot
	for(i in lvl){
		tab1 <- paste0("04_networks/genetics_lvl",i,"-",organ,"_grp_L-corr_pearson_rows-0.01-cut-0.7-full-node_specs.tsv")
		tab2 <- paste0("04_networks/genetics_lvl",i,"-",organ,"_grp_A-corr_pearson_rows-0.01-cut-0.7-full-node_specs.tsv")
		tab <- compare_tables(tab1, tab2, column)
# 		plot_density(tab,i)
# 		plot_boxplot(tab)
		plot_ggboxplot(tab)
		plot_ggboxplot_no_outliers(tab)
	}
}
...
t_cols <- c(transp_cols("cornflowerblue",50),transp_cols("firebrick",50))
solid_cols <- c("blue","red")
pdf(paste0("05_collated_comparisons/lvl",lvl,"-degree_u-H.pdf")); process_column(2,"H"); dev.off()
...
# calc_boxplot_stat <- function(x) { # solution from https://stackoverflow.com/questions/25124895/no-outliers-in-ggplot-boxplot-with-facet-wrap
#   coef <- 1.5
#   n <- sum(!is.na(x))
#   # calculate quantiles
